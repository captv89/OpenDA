...
class CostItem(BaseModel):
    ...
    category: CategoryEnum = Field(
        ...,
        description="Cost category enum",
    )
    ...
    estimated_value: float = Field(
        ...,
        ge=0.0,
        description="Estimated cost amount in the DA currency",
    )
    unit: UnitEnum = Field(
        ...,
        description="Billing unit (e.g. per_movement, lump_sum)",
    )
    quantity: float = Field(
        default=1.0,
        ge=0.0,
        description="Number of units (e.g. 2 towage movements)",
    )
...
class PDASchema(BaseModel):
    ...
    port_call_id: str = Field(
        ...,
        pattern=r"^PC-\d{4}-[A-Z]{5}-\d{4}$",
        description="Unique port call identifier e.g. PC-2025-SGSIN-0042",
        examples=["PC-2025-SGSIN-0042"],
    )
    vessel_name: str = Field(
        ...,
        min_length=1,
        max_length=200,
        description="Full vessel name",
    )
    vessel_imo: str = Field(
        ...,
        pattern=r"^\d{7}$",
        description="IMO number (7 digits, no prefix)",
        examples=["9321483"],
    )
    port_code: str = Field(
        ...,
        pattern=r"^[A-Z]{5}$",
        description="UN/LOCODE (5 uppercase letters)",
        examples=["SGSIN"],
    )
    currency: str = Field(
        ...,
    ...
    estimated_items: list[CostItem] = Field(
        ...,
        min_length=1,
    ...
    total_estimated: float = Field(
        ...,
        ge=0.0,
        description="Declared total — must equal sum of estimated_items values × quantities",
    )
    valid_until: date = Field(
        ...,
        description="Estimate validity expiry date (ISO 8601)",
    )
    prepared_by: str = Field(
        default="Port Agent",
        description="Name or identifier of the agent who prepared the PDA",
    )
